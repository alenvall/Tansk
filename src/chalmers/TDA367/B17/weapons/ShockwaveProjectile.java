...
public class ShockwaveProjectile extends AbstractProjectile {
	...
	public ShockwaveProjectile(int id, Vector2f position) {
		super(id, position, new Vector2f(1,1), 100, 0, 0.5, 1500);
		setSpeed(0.2f);
	...
	public void update(int delta){
		if(getDurationTimer() <= 20 && !activated){
			activated = true;
			for(int i = 0; i < 40; i++){
				AbstractProjectile projectile = new ShockwaveSecondaryProjectile(ServerState.getInstance().generateID(), new Vector2f(getPosition()));
